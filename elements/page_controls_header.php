...
//error_log(date("Y/m/d D H:i:s ",time())."isEditMode!!=".print_r($isEditMode, true)."\r\n",3,"/var/www/test.confeb.net/web/c56/debug_log.txt");//confeb_debug
...
if (!$dh->inDashboard()) {
	...
	//error_log(date("Y/m/d D H:i:s ",time())."page_control_header!!=".print_r($test, true)."\r\n",3,"/var/www/test.confeb.net/web/c56/debug_log.txt");//confeb_debug

	$this->addFooterItem('<script type="text/javascript" src="' . REL_DIR_FILES_TOOLS_REQUIRED . '/i18n_js"></script>');
	if($isEditMode == "true") {
		$this->addHeaderItem($html->javascript('jquery.js'));
	}else{
		$this->addHeaderItem($html->javascript('jquery.js', $currentThemeName));
	}
	$this->addFooterItem($html->javascript('jquery.ui.js'));
	$this->addFooterItem($html->javascript('jquery.form.js'));
	$this->addFooterItem($html->javascript('jquery.rating.js', $currentThemeName));
	if($isEditMode == "true") {
		$this->addFooterItem($html->javascript('bootstrap.js'));
	}else{
		$this->addFooterItem($html->javascript('bootstrap.js', $currentThemeName));
	}
